...
def load_flights(path = "flights/", max_rows = None):#'snow':0.1
    ...
    #distances.drop(["ORIGIN", "DEST"], axis=1, inplace=True)
    #schedule.drop(["OP_CARRIER"], axis=1, inplace=True)
...
print(a.shape)
a.to_csv('airlines_data_dataset_postgres.csv', header=False, index=False, na_rep='', float_format='%.3f')
b.to_csv('schedule_dataset_postgres.csv', header=False, index=False, na_rep='', float_format='%.3f')
c.to_csv('airline_dataset_postgres.csv', header=False, index=False, na_rep='', float_format='%.3f')
