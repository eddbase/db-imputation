    ...
    IF max_range >= 2 THEN
    	    if array_length(categorical_columns_null, 1) > 0 then
    			query := 'CREATE UNLOGGED TABLE ' || output_table_name || '_nullcnt2' || 
             	' PARTITION OF ' || output_table_name ||
             	' FOR VALUES FROM (2) TO (' || array_length(continuous_columns_null, 1) + array_length(categorical_columns_null, 1) + 1 || ') WITH (fillfactor=100)';
             else
            	query := 'CREATE UNLOGGED TABLE ' || output_table_name || '_nullcnt2' || 
             	' PARTITION OF ' || output_table_name ||
             	' FOR VALUES FROM (2) TO (' || array_length(continuous_columns_null, 1) + 1 || ') WITH (fillfactor=100)';
             end if;
    RAISE DEBUG '%', query;
    EXECUTE QUERY;
    END IF;
