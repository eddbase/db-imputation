...
def load_flights(path = "data/flights/", nulls={'CRS_DEP_HOUR':0.25, 'DISTANCE':0.3, 'TAXI_OUT':0.15, 'TAXI_IN':0.1, 'DIVERTED':0.1, 'ARR_DELAY':0.2, 'DEP_DELAY':0.1}, max_rows = None, models = 
...
(((airlines_data["DEP_DELAY"]/scaler3.scale_[c3])+scaler3.data_min_[c3] >= 0)))][["DEP_TIME_HOUR", "DEP_TIME_MIN", "CRS_DEP_HOUR", "CRS_DEP_MIN", "DEP_DELAY", "EXTRA_DAY_DEP"]])
    ...
    distances_dataset = airlines_data.drop_duplicates(subset=["airports"], keep='first', inplace=False, ignore_index=True)[["airports","ORIGIN","DEST","DISTANCE"]]
    schedule_dataset = airlines_data.drop_duplicates(subset=["flight"], keep='first', inplace=False, 
ignore_index=True)[["flight","OP_CARRIER","CRS_DEP_HOUR","CRS_DEP_MIN","CRS_ARR_HOUR","CRS_ARR_MIN","airports"]]


    print("COLS: ", list(airlines_data.columns))

    airlines_data = airlines_data.drop_duplicates(["flight"], inplace=False, 
ignore_index=True)[["DEP_DELAY","TAXI_OUT","TAXI_IN","ARR_DELAY","DIVERTED","ACTUAL_ELAPSED_TIME","AIR_TIME","DEP_TIME_HOUR","DEP_TIME_MIN","WHEELS_OFF_HOUR","WHEELS_OFF_MIN","WHEELS_ON_HOUR","WHEELS_ON_MIN","ARR_TIME_HOUR","ARR_TIME_MIN","MONTH_SIN","MONTH_COS","DAY_SIN","DAY_COS","WEEKDAY_SIN","WEEKDAY_COS","EXTRA_DAY_ARR","EXTRA_DAY_DEP","flight"]]    
    return distances_dataset, schedule_dataset, airlines_data

a,b,c = load_flights()
a["airports"] = a["airports"].astype(int)
b["airports"] = b["airports"].astype(int)
c["DIVERTED"] = c["DIVERTED"].astype('Int64')

a.to_csv('distances_dataset_postgres.csv', header=False, index=False, na_rep='')
b.to_csv('schedule_dataset_postgres.csv', header=False, index=False, na_rep='')
c.to_csv('airline_dataset_postgres.csv', header=False, index=False, na_rep='')
